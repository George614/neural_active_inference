...
def init_weights(init_type, shape, seed, stddev=1.0):
    if init_type == "he_uniform":
        ...
        params = tf.random.uniform(
            shape, minval=-bound, maxval=bound, seed=seed)
...
def ltanh(z):
    ...
    b = 2.0/3.0
    z_scale = z * b
    z_scale = tf.clip_by_value(z_scale, -50.0, 50.0)
...
def sample_uniform(n_s, n_dim):
    eps = tf.random.uniform(shape=(n_s, n_dim), minval=0.0,
                            maxval=1.0, dtype=tf.float32, seed=seed)
    return eps
...
def calc_gaussian_LL(x, mu, sigSqr, log_sigSqr=None):
    ...
    N = x.shape[0]
    delta = (x - mu)
    dist_term = tf.reduce_sum(-(delta * delta) /
                              (sigSqr * 2 + eps), axis=0, keepdims=True)
    var_term = tf.math.log(sigSqr + eps) * (-0.5 * N)
    const_term = tf.math.log(2 * np.pi) * (-0.5 * N)
    # collapse along dimensions
    # compose full log likelihood
    log_likeli = tf.reduce_sum(
        dist_term + var_term, axis=1, keepdims=True) + const_term
    return log_likeli
...
def calc_gaussian_KL(mu1, sigSqr1, mu2, sigSqr2):
    ...
    delta_mu = (mu1 - mu2)
    KL = (tf.math.log(sig2 + eps) - tf.math.log(sig1 + eps)) + \
        (sigSqr1 + (delta_mu * delta_mu))/(sigSqr2 * 2 + eps) - 0.5
...
def fast_log_loss(probs, y_ind_):
    ...
    for i in range(0, y_ind.shape[0]):
        ...
        if ti >= 0:  # entry for masked token, which should be non-negative
            ...
            if py <= 0.0:
                py = 1e-8
            # all other columns in row i ( != ti) are 0, so do nothing
            loss += np.log(py)
...
def neg_cat_loglikeli(y_pred, y_true, epsilon=1e-8, msk=None):
    ...
    nll = -tf.reduce_sum((tf.math.log(tf.clip_by_value(y_pred,
                                                       epsilon, 1.0 - epsilon))) * y_true) / N
    return nll
...
def bce_w_logits(p_logits, x):
    ...
    loss = p_logits - p_logits * x + max_val + \
        tf.math.log(tf.math.exp(-max_val) + tf.math.exp(-p_logits - max_val))
    return loss
