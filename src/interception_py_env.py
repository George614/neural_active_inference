...
class InterceptionEnv(gym.Env):
    ...
    def __init__(
        ...
        self.FPS = 60
        self.lag_coefficient = float(0.5 / self.FPS)
        self.viewer = None
        self.action_type = "speed"  # 'acceleration' or 'speed'
        # type of prior to use: either in obv space or prior space
        self.return_prior = return_prior
        self.use_slope = (
            use_slope  # whether to use slope of ramp to estimate target's fspeed
        )
        # whether to use a prior function that acconuts for target speed change
        self.perfect_prior = perfect_prior
        ...
        if self.action_type == "speed":
            ...
            self.low = np.array(
                [0.0, np.min(self.target_init_speed_list), 0.0, 0.0], dtype=np.float32
            )
            self.high = np.array(
                [
                    self.target_init_distance,
                    self.target_max_speed,
                    self.subject_max_position,
                    self.subject_speed_max,
                ],
                dtype=np.float32,
            )
        else:
    ...
    def step(self, action=None, offset=None):
        ...
        if speed_phase == 1 and target_speed == self.target_final_speed:
            ...
            self.hindsight_error = (
                subject_dis / subject_speed - target_dis / target_speed
            )
        if speed_phase == 1:
            speed_proportion = (
                self.time - self.time_to_change_speed
            ) / self.speed_change_duration
            speed_proportion = np.clip(speed_proportion, 0.0, 1.0)
            target_speed = (
                speed_proportion * (self.target_final_speed - self.target_init_speed)
                + self.target_init_speed
            )
        ...
        if not self.perfect_prior:
            required_speed = first_order_speed
        else:
            if speed_phase == 0:
                if (
                    self.est_time_to_ramp < 0
                ):  # speed change is later than the estimation
                    # assume target will change speed in next frame
                    self.est_time_to_ramp = 1.0 / self.FPS
                target_dis_to_ramp = target_speed * self.est_time_to_ramp
                target_dis_during_ramp = (
                    (target_speed + self.target_fspeed_mean)
                    * 0.5
                    * self.speed_change_duration
                )
                target_TTC_last_part = (
                    target_dis - target_dis_to_ramp - target_dis_during_ramp
                ) / self.target_fspeed_mean
                target_TTC = (
                    target_TTC_last_part
                    + self.speed_change_duration
                    + self.est_time_to_ramp
                )
                ...
                target_dis_ramp = (
                    (target_speed + est_target_fspeed)
                    * 0.5
                    * (self.speed_change_duration - time_past_in_ramp)
                )
                target_TTC_last_part = (
                    target_dis - target_dis_ramp
                ) / est_target_fspeed
                target_TTC = (
                    target_TTC_last_part
                    + self.speed_change_duration
        ...
        if self.action_type == "speed":  # choose pedal position
            if self.return_prior is not None:
                ...
                for prior_action in range(self.action_space.n):
                    ...
                    prior_sub_speed_n = (
                        subject_speed
                        + (pedal_speed_n - subject_speed) * self.lag_coefficient
                    )
                ...
                if offset is None:
                    ...
                    if speed_phase == 2:
                        ...
                        prior_speed_diffs = [
                            np.abs(diff - offset) for diff in prior_speed_diffs
                        ]
                        action_prior = np.argmin(prior_speed_diffs)
                if self.return_prior == "prior_obv":
                    ...
                    prior_sub_speed = (
                        subject_speed
                        + (pedal_speed_prior - subject_speed) * self.lag_coefficient
                    )
                    ...
                    # scale the observations to range (-1, 1)
                    scaled_prior_target_dis = 2 * (
                        prior_target_dis / self.target_init_distance - 0.5
                    )
                    scaled_prior_target_speed = 2 * (
                        target_speed / self.target_max_speed - 0.5
                    )
                    scaled_prior_sub_dis = 2 * (
                        prior_sub_dis / self.subject_max_position - 0.5
                    )
                    scaled_prior_sub_speed = 2 * (
                        prior_sub_speed / self.subject_speed_max - 0.5
                    )
                    prior_scaled_obv = (
                        scaled_prior_target_dis,
                        scaled_prior_target_speed,
                        scaled_prior_sub_dis,
                        scaled_prior_sub_speed,
                    )
        ...
        target_subject_dis = np.sqrt(
            np.square(target_dis)
            + np.square(subject_dis)
            - 2 * target_dis * subject_dis * np.cos(self.approach_angle * np.pi / 180)
        )

        done = bool(
            subject_dis <= 0
            or target_dis <= 0
            or target_subject_dis <= self.intercept_threshold
        )
    ...
    def reset(self):
        ...
        target_subject_dis = np.sqrt(
            np.square(self.target_init_distance)
            + np.square(subject_init_distance)
            - 2
            * self.target_init_distance
            * subject_init_distance
            * np.cos(self.approach_angle * np.pi / 180)
        )
        self.action = None
        self.state = (
            self.target_init_distance,
        ...
        scaled_subject_dis = 2 * (
            subject_init_distance / self.subject_max_position - 0.5
        )
        ...
        self.most_likely_TTCS = (
            self.time_to_change_speed_min + self.time_to_change_speed_max
        ) * 0.5
        # this variable gets updated per step
        self.est_time_to_ramp = self.most_likely_TTCS

        self.hindsight_error = None

        self.info = {"speed_phase": speed_phase, "required_speed": None}
    ...
    def render(self, mode="human", offset=None, isRandom=None):
        ...
        if self.viewer is None:
            ...
            actual_width = int(
                (
                    -math.cos(max(self.approach_angle_list) * math.pi / 180)
                    * self.subject_max_position
                    + self.target_init_distance
                )
            )
            ...
            screen_height = int(
                (
                    math.sin(min(self.approach_angle_list) * math.pi / 180)
                    * self.subject_max_position
                )
                * self.scale
                + 100
            ...
            world_origin = rendering.Transform(
                translation=(self.target_init_distance * self.scale + 10, 90)
            )
            ...
            self.speed_diff_label = text_rendering.Text(
                "Speed difference: {}".format(
                    speed_diff if required_speed is not None else "None"
                )
            )
            ...
            self.TTC_diff_label = text_rendering.Text(
                "TTC difference: {}".format(
                    TTC_diff if TTC_diff is not None else "None"
                )
            )
            info_top -= self.TTC_diff_label.text.content_height
            self.TTC_diff_label.add_attr(rendering.Transform(translation=(5, info_top)))
            self.viewer.add_geom(self.TTC_diff_label)

            if self.action_type == "acceleration":
                self.action_label = text_rendering.Text(
                    "Action (acceleration): "
                    + str(self.action_acceleration_mappings[self.action])
                )
            elif self.action_type == "speed":
                self.action_label = text_rendering.Text(
                    "Pedal speed: {}".format(
                        self.action_speed_mappings[self.action]
                        if self.action is not None
                        else "None"
                    )
                )
            info_top -= self.action_label.text.content_height
            self.action_label.add_attr(rendering.Transform(translation=(5, info_top)))
            self.viewer.add_geom(self.action_label)

            if isRandom is not None:
                self.isRandom_label = text_rendering.Text(
                    "Random action: {}".format("True" if isRandom else "False")
                )
                info_top -= self.isRandom_label.text.content_height
                self.isRandom_label.add_attr(
                    rendering.Transform(translation=(5, info_top))
                )
        ...
        self.target_distance_label.set_text("Target Distance: %.2f" % target_dis)
        self.target_speed_label.set_text("Target Speed: %.2f" % target_speed)
        self.speed_phase_label.set_text("Speed Changing Phase: %d" % speed_phase)
        self.subject_dis_label.set_text("Subject Distance: %.2f" % subject_dis)
        self.subject_speed_label.set_text("Subject Speed: %.2f" % subject_speed)
        if self.action_type == "acceleration":
        ...
        if isRandom is not None:
            self.isRandom_label.set_text(
                "Random action: {}".format("True" if isRandom else "False")
            )
        if offset is not None:
            self.hindsight_label.set_text(
                "Hindsight error: %.2f" % self.hindsight_error
            )
...
if __name__ == "__main__":
    env = InterceptionEnv(
        target_speed_idx=2, approach_angle_idx=3, return_prior="prior_error"
    )
    env.reset()
