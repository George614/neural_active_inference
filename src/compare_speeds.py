...
if __name__ == "__main__":
    ...
    model_save_path = "D:/Projects/neural_active_inference/exp/interception/qai/recogNN_noDelay_InstEpst_HdstBuffer_discount0_record_relu_learnSche_3k/"
    qaiModel1 = load_AIF_agent(6, 2500, model_save_path)
    ...
    for target_speed_idx in [0, 1, 2]:
        ...
        for trial in range(num_trials):
            ...
            ax1.plot(np.arange(len(agent1_speeds)), agent1_speeds, color='green', label='agent', linewidth=0.5)
            ax1.plot(np.arange(len(required_speeds1)), required_speeds1, color='orange', label='required_speed', linewidth=0.5)
            ax1.plot(np.arange(len(first_order_speeds1)), first_order_speeds1, linestyle = '--', color='orange', label='1st-order_speed', linewidth=0.5)
            ...
            ax1.axhline(y=max(env.action_speed_mappings), color = 'red', linestyle = 'dotted', label='max_speed', linewidth=0.5)
            ax1.axhline(y=min(env.action_speed_mappings), color = 'red', linestyle = 'dashed', label='min_speed', linewidth=0.5)
            ax1.legend(bbox_to_anchor=(0., 1.1, 1., .2), loc='lower left', fontsize='xx-small', ncol=3, mode='expand', borderaxespad=0.)
            ax1.set_ylim([0, 15])
            ax1.set_ylabel("Speed")
            ax1.set_xlabel("Time steps")
            ax1.text(0, 13, "agent {}".format("succeed" if succeed1 else "failed", fontsize=5))
            ...
            fig.tight_layout()
            fig.savefig(os.getcwd() + "/plotting/speeds_viz_speedIdx_{}_trial_{}.png".format(target_speed_idx, trial+1), dpi=300, bbox_inches="tight")
            plt.close(fig)