...
model_save_path = (
    "D:/Projects/neural_active_inference/exp/interception/qai/" + folder_names[1] + "/"
)
test_episodes = 30
...
if total_fail_cases != 0:
    print(
        "target_passes_first: {:.2f}%".format(
            100 * target_front_count / total_fail_cases
        )
    )
    print(
        "subject_passes_first: {:.2f}%".format(
            100 * subject_front_count / total_fail_cases
        )
    )
...
with open("{}test_stats_{}.txt".format(model_save_path, agent_name), "w+") as f:
    if total_fail_cases != 0:
        f.write(
            "target_passes_first: {:.2f}%\n".format(
                100 * target_front_count / total_fail_cases
            )
        )
        f.write(
            "subject_passes_first: {:.2f}%\n".format(
                100 * subject_front_count / total_fail_cases
            )
        )
