...
def create_optimizer(opt_type, eta, momentum=0.9, epsilon=1e-08):
    ...
    if opt_type == "nag":
        ...
        optimizer = tf.compat.v1.train.AdamOptimizer(
            learning_rate=eta_v, beta1=0.9, beta2=0.999
        )  # 1e-08)
    elif opt_type == "rmsprop":
        optimizer = tf.compat.v1.train.RMSPropOptimizer(
            learning_rate=eta_v, decay=0.9, momentum=moment_v, epsilon=epsilon
        )  # epsilon=1e-10)
    else:
...
out_dir = args.getArg(
    "out_dir"
)  # "/home/agoroot/IdeaProjects/playful_learning/exp/mcar/act_inf1/"
...
num_episodes = int(
    args.getArg("num_episodes")
)  # 500 # 2000  # total number of training episodes
...
epsilon_by_frame = Linear_schedule(
    epsilon_start, epsilon_final, num_frames * 0.2
)  # linear schedule gives more exploration
...
if record_video:
    ...
    fourcc = cv2.VideoWriter_fourcc(
        *"XVID"
    )  # 'XVID' with '.avi' or 'mp4v' with '.mp4' suffix
    FPS = 30
################################################################################
all_win_mean = []
...
for trial in range(start_trial, n_trials):
    ...
    for ep_idx in range(num_episodes):  # training using episode as cycle
        ...
        if record_stats and ep_idx % record_interval == 0:
            ...
            if record_video:
                video = cv2.VideoWriter(
                    out_dir
                    + "trial_{}_epd_{}_tsidx_{}.avi".format(trial, ep_idx, f_speed_idx),
                    fourcc,
                    float(FPS),
                    (width, height),
                )
        ...
        while not done:
            ...
            if use_env_prior:
                replay_buffer.push(
                    observation, action, reward, next_obv, done, prior=obv_prior
                )
            else:
                        ...
                        if grad is not None:
                            if clip_type == "hard_clip":
                                grad = tf.clip_by_value(
                                    grad, -grad_norm_clip, grad_norm_clip
                                )
                            else:
        ...
        if loss_efe is not None:
            print("-----------------------------------------------------------------")
            print(
                "frame {}, L.efe = {}  eps = {}, rho = {}".format(
                    frame_idx,
                    loss_efe.numpy(),
                    pplModel.epsilon.numpy(),
                    pplModel.rho.numpy(),
                )
            )
        ...
        print(
            "episode {}, r.mu = {:.3f}  win.mu = {:.3f}".format(
                ep_idx + 1, episode_reward, reward_window_mean
            )
        )
        print("-----------------------------------------------------------------")
        if ep_idx % 50 == 0:
    ...
    if record_stats:
        ...
        np.save(
            "{0}trial_{1}_EFE_values.npy".format(out_dir, trial), EFE_values_trial_list
        )
        print("==> Saving TTC_diff sequence...")
        np.save(
            "{0}trial_{1}_TTC_diffs.npy".format(out_dir, trial),
            np.asarray(TTC_diff_list),
        )
        with open("{}who_passes_first.txt".format(out_dir), "a+") as f:
            total_fail_cases = target_front_count + subject_front_count
            f.write(
                "trial_{}_target_passes_first: {:.2f}%\n".format(
                    trial, 100 * target_front_count / total_fail_cases
                )
            )
            f.write(
                "trial_{}_subject_passes_first: {:.2f}%\n".format(
                    trial, 100 * subject_front_count / total_fail_cases
                )
            )
...
if plot_rewards:
    ...
    for tr in range(len(all_win_mean)):
        fig = plt.figure()
        line = plt.plot(
            np.arange(len(all_win_mean[tr])), all_win_mean[tr], linewidth=0.5
        )
        plt.xlabel("Episodes")
