...
if create_video:
    video = cv2.VideoWriter(
        model_save_path
        + "trial_{}_epd_{}_tsidx_{}.avi".format(
            trial_num, episode_num, target_speed_idx
        ),
        fourcc,
        float(FPS),
        (width, height),
...
if plot_efe:
    ...
    ani.save(
        os.path.join(
            model_save_path,
            "EFE_animation_trial_{}_epd_{}_tsidx_{}.avi".format(
                trial_num, episode_num, target_speed_idx
            ),
        ),
        fps=FPS,
...
if combine_videos:
    ...
    cmd_traj = [
        "ffmpeg",
        "-i",
        "trial_{}_epd_{}_tsidx_{}.avi".format(trial_num, episode_num, target_speed_idx),
        "-vf",
        "scale=640:-1",
        "trajectory_trial_{}_epd_{}_tsidx_{}.mp4".format(
            trial_num, episode_num, target_speed_idx
        ),
    ]
    cmd_efe = [
        ...
        "-vf",
        "scale=640:-1",
        "EFE_animation_trial_{}_epd_{}_tsidx_{}_scaled.mp4".format(
            trial_num, episode_num, target_speed_idx
        ),
    ]
    cmd_combine = [
        ...
        "-filter_complex",
        "vstack=inputs=2",
        "combined_trial_{}_epd_{}_tsidx_{}.mp4".format(
            trial_num, episode_num, target_speed_idx
        ),
    ]
