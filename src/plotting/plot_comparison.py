...
exp_dir_list = [
    "DQN_noDelay_reward_discount0.0_pedal1.0_relu_learnSche_3k",
...
fig.savefig(
    os.getcwd() + "/DQN_AIFInstOnly_InstEpst_compare.png", dpi=300, bbox_inches="tight"
)
...
def plot_rewards_subplots(grouped_dirs):
    ...
    def plot_rewards_compare(
        input_dirs, ax, title=None, colors=None, shade_colors=None, labels=None
    ):
        win_reward_np_list = []
        for input_dir in input_dirs:
            result_dir = Path(input_dir)
    ...
    fig, axs = plt.subplots(
        2, 2, figsize=(8, 6), sharey=True, sharex=True, constrained_layout=True
    )
    trans = mtransforms.ScaledTranslation(-20 / 72, 7 / 72, fig.dpi_scale_trans)
    ax00 = plot_rewards_compare(
        grouped_dirs[0],
        axs[0, 0],
        title=r"$\gamma=0.0, K=1.0$",
        colors=["red", "blue", "green"],
        shade_colors=["pink", "cyan", "lime"],
        labels=["DQN_Reward", "AIF_InstOnly", "AIF_InstEpst"],
    )
    ax00.text(
        0.0,
        1.0,
        "A",
        transform=ax00.transAxes + trans,
        fontsize="medium",
        va="bottom",
        fontfamily="serif",
    )
    ax01 = plot_rewards_compare(
        grouped_dirs[1],
        axs[0, 1],
        title=r"$\gamma=0.0, K=0.5$",
        colors=["red", "blue", "green"],
        shade_colors=["pink", "cyan", "lime"],
        labels=["DQN_Reward", "AIF_InstOnly", "AIF_InstEpst"],
    )
    ax01.text(
        ...
        1.0,
        "B",
        transform=ax01.transAxes + trans,
        fontsize="medium",
        va="bottom",
        fontfamily="serif",
    )
    ax01.get_legend().remove()
    ax10 = plot_rewards_compare(
        grouped_dirs[2],
        axs[1, 0],
        title=r"$\gamma=0.99, K=1.0$",
        colors=["red", "blue", "green"],
        shade_colors=["pink", "cyan", "lime"],
        labels=["DQN_Reward", "AIF_InstOnly", "AIF_InstEpst"],
    )
    ax10.text(
        ...
        1.0,
        "C",
        transform=ax10.transAxes + trans,
        fontsize="medium",
        va="bottom",
        fontfamily="serif",
    )
    ax10.get_legend().remove()
    ax11 = plot_rewards_compare(
        grouped_dirs[3],
        axs[1, 1],
        title=r"$\gamma=0.99, K=0.5$",
        colors=["red", "blue", "green"],
        shade_colors=["pink", "cyan", "lime"],
        labels=["DQN_Reward", "AIF_InstOnly", "AIF_InstEpst"],
    )
    ax11.text(
        ...
        1.0,
        "D",
        transform=ax11.transAxes + trans,
        fontsize="medium",
        va="bottom",
        fontfamily="serif",
    )
    ...
    fig.savefig(
        os.getcwd() + "/DQN_AIFInstOnly_InstEpst_rewards_compare.jpg",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
