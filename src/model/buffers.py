...
"""
Various experience replay modules used in reinforcement learning.
...
class ReplayBuffer(object):
    ...
    def push(self, state, action, reward, next_state, done, R_te=None, prior=None):
        ...
        if R_te is not None and prior is not None:
            self.buffer.append(
                (state, action, reward, next_state, done, R_te, prior))
        elif R_te is None and prior is not None:
            self.buffer.append(
                (state, action, reward, next_state, done, prior))
    ...
    def sample(self, batch_size):
        if self.store_Rte and self.store_prior:
            state, action, reward, next_state, done, R_te, prior = zip(
                *random.sample(self.buffer, batch_size))
        elif self.store_Rte and not self.store_prior:
            state, action, reward, next_state, done, R_te = zip(
                *random.sample(self.buffer, batch_size))
        elif not self.store_Rte and self.store_prior:
            state, action, reward, next_state, done, prior = zip(
                *random.sample(self.buffer, batch_size))
        else:
    ...
    def push(self, state, action, reward, next_state, done, R_te, prior=None):
        ...
        if len(self.buffer) < self.capacity:
            if self.store_prior:
                ...
                self.buffer.append(
                    (state, action, reward, next_state, done, R_te))
        else:
            if self.store_prior:
